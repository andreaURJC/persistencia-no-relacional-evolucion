...
public interface VueloRepository extends JpaRepository<Vuelo, Long> {
    ...
    @Query("SELECT new com.urjc.plains.dtos.VuelosPorCiudadDestinoYFechaDTO(v.codigoVuelo, v.compania, v.aeropuertoOrigen.iata, v.aeropuertoDestino.iata, v.fechaHora, v.duracion)" +
            "FROM Vuelo v JOIN Aeropuerto a ON v.aeropuertoDestino.iata = a.iata WHERE a.ciudad LIKE :ciudadDestino AND FUNCTION('date_format', v.fechaHora, '%Y-%m-%d') = :fecha ORDER BY FUNCTION('date_format', v.fechaHora, '%H')")
